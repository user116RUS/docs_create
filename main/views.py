...
def create_org(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            for file in files:
                # Do something with the file (e.g., save to a specific location,
                # associate with the Organization model, etc.)
                print(f"Uploaded file: {file.name}")

            # Prepare a JSON response
            response_data = {'message': 'Организация успешно создана и файлы загружены!'}
            return HttpResponse(json.dumps(response_data), content_type='application/json')

            messages.success(request, 'Организация успешно создана!')
            return redirect('main:index')  # Replace with your actual URL

        else:
            messages.error(request, 'Пожалуйста, исправьте ошибки в форме.')

            # Prepare a JSON response for form errors
